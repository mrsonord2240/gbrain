...
export class PostgresEngine implements BrainEngine {
  ...
  private async applyForwardReferenceBootstrap(injectedConn?: postgres.Sql): Promise<void> {
    ...
    // v0.42.x (v121): timeline_entries.event_page_id, the Life Chronicle
    // event→timeline projection pointer. idx_timeline_event_page and
    // idx_timeline_event_dedup in SCHEMA_SQL reference it unconditionally, so
    // on a brain that already has timeline_entries (CREATE TABLE IF NOT
    // EXISTS is then a no-op), the blob's CREATE INDEX crashes with "column
    // event_page_id does not exist" before runMigrations() gets a chance to
    // add it via v121. Bootstrap adds the column first. v121 runs later via
    ...
    if (needsTimelineEventPageId) {
      // v121 (timeline_entries_event_page_id): the FK + indexes land via the
      // blob (CREATE INDEX IF NOT EXISTS) and v121 (FK guarded DO block);
      // bootstrap only adds the column so the blob's CREATE INDEX doesn't
      // crash. v121 runs later via runMigrations and is idempotent.
      await conn.unsafe(`
