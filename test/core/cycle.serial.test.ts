...
describe('runCycle — yieldBetweenPhases hook', () => {
  ...
  test('hook is called between every phase', async () => {
    ...
    // #2653 + inbox_triage: 24 phases (added `drift` between calibration_profile
    // and conversation_facts_backfill, and `inbox_triage` between enrich_thin
    // and skillopt — both landed in this merge).
  ...
  test('hook exceptions do not abort the cycle', async () => {
    ...
    // #2653 + inbox_triage: 24 phases (+drift, +inbox_triage).
