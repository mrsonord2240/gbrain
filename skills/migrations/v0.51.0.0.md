...
This is an operator-approved coordinated topology upgrade, not an automatic repair
for `owner_unavailable`. Startup, scheduled maintenance and `doctor --fix` must
stop at read-only writer status and ask the operator before changing ownership or
managed mode. Follow the state-bound administration procedure in
`docs/architecture/topologies.md`; do not run it just because a data-path error
mentions missing ownership.
...
Inspect every affected source, owner epoch, native capability and outstanding recovery record. Retain the `admin_state` fingerprint from reviewed writer status, and inspect again after every change. A stale fingerprint refuses rather than applying to changed topology. Resolve recovery before moving a source or activating. Intentional DB-only sources remain DB-only.
...
The intent must match the action; `--confirm-quiesced` alone cannot activate. A TTY
and generic `--yes` are not authorization. Deliberate noninteractive provisioning
uses the same reviewed preconditions; stdio and remote callers remain refused.
